...
public class Forum {
	...
	public Forum searchSpec(String forumURL){
		...
		Node title0Node = (Node) startNode1.getFirstChild().getChildren().elementAt(1);
		forum.title = title0Node.toPlainTextString().replace("\r", "").replace("\n", "").replace("\t", "").replace("&nbsp;", " ");
		Node nameNode = (Node) startNode1.getFirstChild().getChildren().elementAt(3);
		forum.author = nameNode.toPlainTextString().replace("\r", "").replace("\n", "").replace("\t", "").replace("&nbsp;", " ");
		Node dateNode = (Node) startNode1.getFirstChild().getChildren().elementAt(5);
		forum.date = Date.parseDate(dateNode.toPlainTextString().replace("\r", "").replace("\n", "").replace("\t", "").replace("&nbsp;", " "));
